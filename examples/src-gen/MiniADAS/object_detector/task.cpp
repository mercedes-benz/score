...
static constexpr std::string_view kLogPrefix{"object_detector"};


// Task class definition for containing Runnable Instance 'object_detector'
class Object_detectorTask final : public score::middleware::TaskBase<Object_detectorInputReferences, NeuralNetInference> {
  public:
    using base_type = score::middleware::TaskBase<Object_detectorInputReferences, NeuralNetInference>;
  ...
  protected:
    ...
    std::error process([[maybe_unused]] const std::chrono::time_point& time,
      ...
      std::vector<const adas::perception::CameraFrame*> input_frames_received(2);
        for (const score::comm::SampleReference& sample_ref : inputs.input_frames_sample_refs) {
            this->input_frames_buffer_.getEntry(sample_ref.slot())
                .with_value([&input_frames_received](score::base::ReferenceWrapper<const score::comm::SampleBufferEntry<adas::perception::CameraFrame, score::comm::kDefaultPageSize>>& sample_data) {
                    input_frames_received.push_back(&sample_data.get().sample.get());
                })
        ...
        NeuralNetInferenceInputs input_buffers {
            std::queue{ input_frames_received }
        };

        // Containers for the output processing for-loops
        using detections_vector_element_type =
            std::pair<score::comm::SampleReference::slot_type, score::comm::ShmMutableSampleBufferBase<adas::perception::ObjectDetections, 4>::mutable_sample_type>;
        std::vector<detections_vector_element_type> detections_vector(1);

        // Construct the output data structure for the runnable
        // NOTE: relying on order instead of using designated initializer (since this is a C++20 feature)
        NeuralNetInferenceOutputs output_buffers{
            std::queue<score::comm::ShmMutableSampleBufferBase<adas::perception::ObjectDetections, 4>, 1> {
                detections_buffer_,
                detections_vector
            }
        };
        ...
        if (onupdate_result.success()) {
            for (detections_vector_element_type& pair : detections_vector) {
                pair.second.publish(time, detections_sequence_counter_);
                outputs.push(score::comm::SampleReference{ kDetectionsTopicId, pair.first, time, detections_sequence_counter_});
                detections_sequence_counter_++;
            }
        }
        return onupdate_result;
    }

    std::error release([[maybe_unused]] typename score::middleware::TaskData<Object_detectorInputReferences>::sample_release_queue& release_samples) noexcept final {
  ...
  private:
    ...
    static constexpr score::comm::TopicId kInputFramesTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0x8cf3660bce5f01a7, 0 };
    score::comm::ShmSampleBuffer<adas::perception::CameraFrame, 3> input_frames_buffer_{0};

    // We need a Counter and Buffer for each member in struct Outputs
    score::comm::SampleReference::sequence_number_type detections_sequence_counter_{0};
    static constexpr score::comm::TopicId kDetectionsTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0xd75d18345a52fdf9, 0 };
    score::comm::ShmMutableSampleBuffer<adas::perception::ObjectDetections, 4> detections_buffer_{0};
};
...
static Object_detectorTask task{};
...
int main(int argc, char* argv[]) {
    ...
    score::log::Logger::registerStandardSink(log_level, std::string_view{"Object_detectorTASK"});
    ...
    // initialize task
    std::error score_result{task.init(score::middleware::PortName{"MiniADAS", "NeuralNetInference", "object_detector"})};
