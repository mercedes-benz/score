...
static constexpr std::string_view kLogPrefix{"traffic_sign_controller"};


// Task class definition for containing Runnable Instance 'traffic_sign_controller'
class Traffic_sign_controllerTask final : public score::middleware::TaskBase<Traffic_sign_controllerInputReferences, TrafficSignRecognition> {
  public:
    using base_type = score::middleware::TaskBase<Traffic_sign_controllerInputReferences, TrafficSignRecognition>;
  ...
  protected:
    ...
    std::error process([[maybe_unused]] const std::chrono::time_point& time,
      ...
      std::vector<const adas::perception::ObjectDetections*> object_detections_received(1);
        ...
        TrafficSignRecognitionInputs input_buffers {
            std::queue{ object_detections_received }
        };

        // Containers for the output processing for-loops
        using traffic_sign_commands_vector_element_type =
            std::pair<score::comm::SampleReference::slot_type, score::comm::ShmMutableSampleBufferBase<adas::perception::TrafficSignCommand, 2>::mutable_sample_type>;
        ...
        // NOTE: relying on order instead of using designated initializer (since this is a C++20 feature)
        TrafficSignRecognitionOutputs output_buffers{
            std::queue<score::comm::ShmMutableSampleBufferBase<adas::perception::TrafficSignCommand, 2>, 1> {
                traffic_sign_commands_buffer_,
        ...
        if (onupdate_result.success()) {
            for (traffic_sign_commands_vector_element_type& pair : traffic_sign_commands_vector) {
                pair.second.publish(time, traffic_sign_commands_sequence_counter_);
                outputs.push(score::comm::SampleReference{ kTrafficSignCommandsTopicId, pair.first, time, traffic_sign_commands_sequence_counter_});
                traffic_sign_commands_sequence_counter_++;
            }
        }
        return onupdate_result;
    }

    std::error release([[maybe_unused]] typename score::middleware::TaskData<Traffic_sign_controllerInputReferences>::sample_release_queue& release_samples) noexcept final {
  ...
  private:
    ...
    score::comm::SampleReference::sequence_number_type traffic_sign_commands_sequence_counter_{0};
    static constexpr score::comm::TopicId kTrafficSignCommandsTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0x585aaf8642d74510, 0 };
    score::comm::ShmMutableSampleBuffer<adas::perception::TrafficSignCommand, 2> traffic_sign_commands_buffer_{0};
};
...
static Traffic_sign_controllerTask task{};
...
int main(int argc, char* argv[]) {
    ...
    score::log::Logger::registerStandardSink(log_level, std::string_view{"Traffic_sign_controllerTASK"});
    ...
    // initialize task
    std::error score_result{task.init(score::middleware::PortName{"MiniADAS", "TrafficSignRecognition", "traffic_sign_controller"})};
