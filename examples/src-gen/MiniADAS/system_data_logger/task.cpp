...
static constexpr std::string_view kLogPrefix{"system_data_logger"};


// Task class definition for containing Runnable Instance 'system_data_logger'
class System_data_loggerTask final : public score::middleware::TaskBase<System_data_loggerInputReferences, SystemLogger> {
  public:
    using base_type = score::middleware::TaskBase<System_data_loggerInputReferences, SystemLogger>;
  ...
  protected:
    ...
    std::error process([[maybe_unused]] const std::chrono::time_point& time,
      ...
      std::vector<const adas::perception::CameraFrame*> camera_data_log_received(5);
      std::vector<const adas::perception::ObjectDetections*> detection_log_received(5);
      std::vector<const adas::perception::BrakeCommand*> brake_command_log_received(10);
      std::vector<const adas::perception::TrafficSignCommand*> traffic_sign_log_received(5);
        for (const score::comm::SampleReference& sample_ref : inputs.camera_data_log_sample_refs) {
            this->camera_data_log_buffer_.getEntry(sample_ref.slot())
                .with_value([&camera_data_log_received](score::base::ReferenceWrapper<const score::comm::SampleBufferEntry<adas::perception::CameraFrame, score::comm::kDefaultPageSize>>& sample_data) {
                    camera_data_log_received.push_back(&sample_data.get().sample.get());
                })
        ...
        for (const score::comm::SampleReference& sample_ref : inputs.detection_log_sample_refs) {
            this->detection_log_buffer_.getEntry(sample_ref.slot())
                .with_value([&detection_log_received](score::base::ReferenceWrapper<const score::comm::SampleBufferEntry<adas::perception::ObjectDetections, score::comm::kDefaultPageSize>>& sample_data) {
                    detection_log_received.push_back(&sample_data.get().sample.get());
                })
                .with_error([&sample_ref](const std::error& error) {
        ...
        for (const score::comm::SampleReference& sample_ref : inputs.brake_command_log_sample_refs) {
            this->brake_command_log_buffer_.getEntry(sample_ref.slot())
                .with_value([&brake_command_log_received](score::base::ReferenceWrapper<const score::comm::SampleBufferEntry<adas::perception::BrakeCommand, score::comm::kDefaultPageSize>>& sample_data) {
                    brake_command_log_received.push_back(&sample_data.get().sample.get());
                })
                .with_error([&sample_ref](const std::error& error) {
        ...
        for (const score::comm::SampleReference& sample_ref : inputs.traffic_sign_log_sample_refs) {
            this->traffic_sign_log_buffer_.getEntry(sample_ref.slot())
                .with_value([&traffic_sign_log_received](score::base::ReferenceWrapper<const score::comm::SampleBufferEntry<adas::perception::TrafficSignCommand, score::comm::kDefaultPageSize>>& sample_data) {
                    traffic_sign_log_received.push_back(&sample_data.get().sample.get());
                })
                .with_error([&sample_ref](const std::error& error) {
        ...
        SystemLoggerInputs input_buffers {
            std::queue{ camera_data_log_received },
            std::queue{ detection_log_received },
            std::queue{ brake_command_log_received },
            std::queue{ traffic_sign_log_received }
        };

        // Containers for the output processing for-loops
        using log_entries_vector_element_type =
            std::pair<score::comm::SampleReference::slot_type, score::comm::ShmMutableSampleBufferBase<adas::perception::LogEntry, 2>::mutable_sample_type>;
        std::vector<log_entries_vector_element_type> log_entries_vector(10);

        // Construct the output data structure for the runnable
        // NOTE: relying on order instead of using designated initializer (since this is a C++20 feature)
        SystemLoggerOutputs output_buffers{
            std::queue<score::comm::ShmMutableSampleBufferBase<adas::perception::LogEntry, 2>, 10> {
                log_entries_buffer_,
                log_entries_vector
            }
        };
        ...
        if (onupdate_result.success()) {
            for (log_entries_vector_element_type& pair : log_entries_vector) {
                pair.second.publish(time, log_entries_sequence_counter_);
                outputs.push(score::comm::SampleReference{ kLogEntriesTopicId, pair.first, time, log_entries_sequence_counter_});
                log_entries_sequence_counter_++;
            }
        }
        return onupdate_result;
    }

    std::error release([[maybe_unused]] typename score::middleware::TaskData<System_data_loggerInputReferences>::sample_release_queue& release_samples) noexcept final {
  ...
  private:
    ...
    static constexpr score::comm::TopicId kCameraDataLogTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0x8cf3660bce5f01a7, 0 };
    score::comm::ShmSampleBuffer<adas::perception::CameraFrame, 3> camera_data_log_buffer_{0};
    static constexpr score::comm::TopicId kDetectionLogTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0xd75d18345a52fdf9, 0 };
    score::comm::ShmSampleBuffer<adas::perception::ObjectDetections, 4> detection_log_buffer_{0};
    static constexpr score::comm::TopicId kBrakeCommandLogTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0x502077a899668e1c, 0 };
    score::comm::ShmSampleBuffer<adas::perception::BrakeCommand, 2> brake_command_log_buffer_{0};
    static constexpr score::comm::TopicId kTrafficSignLogTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0x585aaf8642d74510, 0 };
    score::comm::ShmSampleBuffer<adas::perception::TrafficSignCommand, 2> traffic_sign_log_buffer_{0};

    // We need a Counter and Buffer for each member in struct Outputs
    score::comm::SampleReference::sequence_number_type log_entries_sequence_counter_{0};
    static constexpr score::comm::TopicId kLogEntriesTopicId{ score::comm::SampleStorageType::kShmSampleBuffer, 0xfac144cc5b0c0154, 0 };
    score::comm::ShmMutableSampleBuffer<adas::perception::LogEntry, 2> log_entries_buffer_{0};
};
...
static System_data_loggerTask task{};
...
int main(int argc, char* argv[]) {
    ...
    score::log::Logger::registerStandardSink(log_level, std::string_view{"System_data_loggerTASK"});
    ...
    // initialize task
    std::error score_result{task.init(score::middleware::PortName{"MiniADAS", "SystemLogger", "system_data_logger"})};
